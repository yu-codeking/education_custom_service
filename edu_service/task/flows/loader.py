...
from edu_service.task.flows.flows import Flow, FlowList, FlowSlot
from edu_service.task.flows.steps import CollectionFlowStep, FlowStep
...
class FlowLoader:
    ...
    def _load_flows(
        self, flow_dict: dict[str, Any], loaded_slots: dict[str, FlowSlot]
    ) -> list[Flow]:
    ...
    def _build_flow_slot(
        self, steps: list[FlowStep], loaded_slots: dict[str, FlowSlot]
    ) -> dict[str, FlowSlot]:
        """
...
if __name__ == "__main__":
    ...
    flow_list = flow_loader.load_multi_yaml(
        [Path("system_flows.yml"), Path("user_flows.yml")]
    )
