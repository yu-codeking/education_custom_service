...
class ActionSearchCourseInfo(Action):
    ...
    async def run(self, action_kwargs: dict[str, Any], state: DialogueState) -> ActionResult:
        ...
        series_title = str((detail or {}).get("seriesName") or best.get("seriesName") or keyword)

        overview_parts: list[str] = []
        if detail:
            description = str(detail.get("description") or "").strip().replace("\n", "")
            if description:
                overview_parts.append(f"课程简介：{description[:100]}{'…' if len(description) > 100 else ''}")
            mode = delivery_mode_name(detail.get("deliveryModeCode"))
            score = detail.get("avgScore")
            overview_parts.append(f"授课方式：{mode}，学员评分 {float(score):.1f} 分" if score is not None else f"授课方式：{mode}")
        ...
        updated_slots = {
            ...
            "course_overview": "。".join(part.strip() for part in overview_parts if part.strip()) + ("。" if overview_parts else ""),
            "course_cohorts": (f"在售班次：" + "；".join(cohort_parts) + "。") if cohort_parts else "目前没有可报名的班次。",
        }
