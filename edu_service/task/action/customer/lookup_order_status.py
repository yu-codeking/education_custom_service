...
def _build_order_summary(detail: dict[str, Any], item_name: str | None) -> str:
    ...
    if refund_summary.get("refundRequestCount"):
        parts.append(
            f"已有 {refund_summary['refundRequestCount']} 笔退款记录，退款 ¥{refund_summary.get('refundAmount', 0)}"
        )
...
class ActionLookupOrderStatus(Action):
    ...
    async def run(
        ...
        status_text = order_status_name(
            detail.get("orderStatusCode") or order_brief.get("orderStatusCode")
        )
