...
def _build_progress_summary(cohort_name: str, progress: dict[str, Any]) -> str:
    ...
    if total_sessions:
        lines.append(
            f"· 出勤：到课 {present_count} 次 / 缺勤 {absent_count} 次（共排课 {total_sessions} 次）"
        )
    else:
    ...
    if total_videos:
        ...
        lines.append(
            f"· 视频：完成 {completed_videos}/{total_videos} 个（累计观看约 {watched_minutes} 分钟）"
        )
    ...
    async def run(
        ...
        if matched is None:
            candidate_names = "、".join(
                str(c.get("cohortName")) for c in my_cohorts[:3]
            )
            hint = (
                f"我这里有你的这些班次：{candidate_names}。" if candidate_names else ""
            )
            return ActionResult(
                updated_slots={
        ...
        enroll_status_code = str(
            progress.get("enrollStatusCode") or matched.get("enrollStatusCode") or ""
        )
