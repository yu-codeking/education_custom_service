...
class ActionCreateTicket(Action):
    ...
    async def run(self, action_kwargs: dict[str, Any], state: DialogueState) -> ActionResult:
        ...
        ticket_type = normalize_ticket_type(str(slots.get("ticket_type") or "") + description)

        profile = await get_student_profile(state)
        if profile is None or profile.get("studentId") is None:
            return ActionResult(updated_slots={"ticket_summary": _fallback_summary("当前账号还没有学员档案。")})

        order_brief = await find_order_by_no(order_number, state) if order_number else None
        ...
        if not items:
            return ActionResult(updated_slots={"ticket_summary": _fallback_summary("该订单下找不到可关联的课程明细。")})
        ...
        title = description[:20] if description else f"{TICKET_TYPE_NAMES[ticket_type]}工单"
        ...
        return ActionResult(updated_slots={"ticket_summary": _fallback_summary(result.message or "服务暂时不可用")})
