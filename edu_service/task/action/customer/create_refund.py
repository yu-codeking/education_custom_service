...
class ActionCreateRefund(Action):
    ...
    async def run(self, action_kwargs: dict[str, Any], state: DialogueState) -> ActionResult:
        ...
        refund_type = normalize_refund_type(str(slots.get("refund_type") or "") + reason)
        ...
        refundable = [item for item in items if item.get("orderItemStatusCode") in ("paid", "completed")]
        if not refundable:
            return ActionResult(updated_slots={
