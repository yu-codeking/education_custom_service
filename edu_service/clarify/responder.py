...
class ClarifyResponder:
    async def respond(
        self, reason: ClarifyReason, dialogue_state: DialogueState
    ) -> list[BotMessage]:
        """
    ...
    def _build_prompt_inputs(
        self, reason: ClarifyReason, state: DialogueState
    ) -> dict[str, Any]:
        ...
        history_str = ChatHistoryBuilder.build(current_session.turns[-10:])
        focused_object_json = (
    ...
    def build_clarify_message(
        ...
        if reason is ClarifyReason.OBJECT_REQUIRES_INTENT:
            ...
            if focused_object is not None and focused_object.type == "order":
                return (
                    "我已经收到这个订单了。你想查订单状态、申请退费，还是提交工单呢？"
                )
